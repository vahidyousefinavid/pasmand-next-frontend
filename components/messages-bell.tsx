 ...
 *
 * It sits next to the bell for a reason: a notification about a message and the
 * message itself are two different things, and the bell only ever showed the
 * first. Someone who read the notification card and dismissed it had no way
 * back to the conversation.
 *
 * The count is refreshed on every navigation rather than polled — the app is a
 * handful of screens and moving between them is the moment the number matters.
 */
export default function MessagesBell() {
  const [unread, setUnread] = useState(0);
  const pathname = usePathname();
  // After mounting, not during render — see the note in notification-center.
  const [signedIn, setSignedIn] = useState(false);
  useEffect(() => { setSignedIn(Boolean(Cookies.get('auth_token'))); }, []);

  useEffect(() => {
    if (!signedIn) return;
    fetch('/api/v1/chat/unread', { credentials: 'include' })
