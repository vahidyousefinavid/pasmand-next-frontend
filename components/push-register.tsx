 ...
 *
 * A citizen who is not subscribed never hears that a collector took their
 * request or is on the way — the two moments they are actually waiting on — so
 * the prompt is not buried in settings. It is still not fired on page load: a permission dialog that
...
async function subscribe(): Promise<boolean> {
  ...
  await axiosService({
    url: '/api/v1/subscription/saveSubscription',
    method: 'post',
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ margin: 0, fontSize: S.sm, fontWeight: 800, color: C.textStrong }}>
          از وضعیت درخواستتان باخبر شوید
        </p>
        <p style={{ margin: '5px 0 0', fontSize: S.xs, color: C.muted, lineHeight: 1.85 }}>
          وقتی جمع‌آور درخواست شما را می‌پذیرد یا در راه است، حتی با برنامهٔ بسته خبردار می‌شوید.
        </p>
        <button
          type="button"
          onClick={enable}
