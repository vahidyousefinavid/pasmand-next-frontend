...
export default function SecondStep({ onNext, onBack }: SecondStepProps) {
  const [selectedLocation, setSelectedLocation] = useState<{ lat: number; lng: number } | null>(null);
  const [address, setAddress] = useState('');
  const [suggestions, setSuggestions] = useState<NeshanSearchItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [geoLocationError, setGeoLocationError] = useState<string>('');

  const { selectedCity } = useCity();

  const defaultCenter = { lat: selectedCity?.lat || 35.6892, lng: selectedCity?.lng || 51.3890 }; // تهران

  // API Keys (بهتره از env بیاری؛ فعلاً مستقیم گذاشتم)
  const SEARCH_API_KEY = "service.891bcedbc31543bd83cceee7f2dd0610"; // برای search/reverse

  // ۱) کلیک روی نقشه → Reverse Geocoding نشان
  const handleLocationSelect = async (latlng: { lat: number; lng: number }) => {
    setSelectedLocation(latlng);
    setLoading(true);
    try {
      ...
      console.error(err);
      setAddress('خطا در دریافت آدرس از سرویس نشان');
    } finally {
      setLoading(false);
    }
  };

  // ۲) تایپ آدرس → Geocoding نشان (search)
  const handleAddressChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setAddress(value);
    ...
    if (value.length > 2) {
      setLoading(true);
      try {
        // نمونه ساده از search:
        // GET https://api.neshan.org/v6/search?term=...&city=تهران
        const url = `https://api.neshan.org/v6/search?term=${encodeURIComponent(
          value
        )}&city=${encodeURIComponent("تهران")}`;
        ...
        const data = await response.json();
        // ساختار خروجی v6 معمولا شامل:
        // { "count": 3, "items": [ { title, address, location: {x, y}, ... }, ... ] }
  ...
  // ۳) کلیک روی suggestion → بردن مارکر و قرار دادن آدرس
  const handleSuggestionClick = (suggestion: NeshanSearchItem) => {
    const latlng = { lat: suggestion.location.y, lng: suggestion.location.x };
    setSelectedLocation(latlng);
    // می‌توانی فقط عنوان یا آدرس را بگذاری؛ من ترکیبی گذاشتم:
    setAddress(`${suggestion.title} - ${suggestion.address}`);
    setSuggestions([]);
  };

  // ۴) گرفتن موقعیت فعلی از مرورگر → سپس reverse نشان
  const getCurrentLocation = () => {
    setGeoLocationError('');
    setLoading(true);
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        setSelectedLocation(latlng);
        await handleLocationSelect(latlng); // reverse از نشان
        setLoading(false);
      },
      (error) => {
        let errorMessage = 'خطا در دریافت موقعیت مکانی';
