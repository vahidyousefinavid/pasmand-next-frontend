...
import {
  ArrowLeftCircle, Bell, CalendarCheck, CheckCheck, CheckCircle2, FileText, Inbox, Info, Loader2,
  Megaphone, MessageSquare, Wallet,
} from 'lucide-react';
...
type Kind =
  ...
  // خدمات شهر — only ever arrive in a city that runs the module they belong to.
  | 'report' | 'letter' | 'booking';
/** Straight from the service catalogue, so a module is one colour everywhere. */
const SERVICE_COLOUR = { report: '#c2790b', letter: '#2563eb', booking: '#7c4dcc' };
...
const KIND: Record<Kind, { label: string; icon: React.ReactNode; color: string }> = {
  ...
  request_new: { label: 'درخواست تازه', icon: <Inbox className="h-4 w-4" />, color: C.green },
  request_accepted: { label: 'پذیرش درخواست', icon: <CheckCircle2 className="h-4 w-4" />, color: C.green },
  request_status: { label: 'وضعیت درخواست', icon: <Info className="h-4 w-4" />, color: C.amber },
  wallet: { label: 'کیف پول', icon: <Wallet className="h-4 w-4" />, color: C.statusOk },
  report: { label: 'گزارش ۱۳۷', icon: <Megaphone className="h-4 w-4" />, color: SERVICE_COLOUR.report },
  letter: { label: 'کارتابل', icon: <FileText className="h-4 w-4" />, color: SERVICE_COLOUR.letter },
  booking: { label: 'رزرو', icon: <CalendarCheck className="h-4 w-4" />, color: SERVICE_COLOUR.booking },
};
...
function destination(url: string): string {
  ...
  if (url.startsWith('/history')) return 'مشاهدهٔ درخواست';
  if (url.startsWith('/wallet')) return 'مشاهدهٔ کیف پول';
  if (url.startsWith('/profile')) return 'مشاهدهٔ پروفایل';
  if (url.startsWith('/reports')) return 'مشاهدهٔ گزارش';
  if (url.startsWith('/cartable')) return 'مشاهدهٔ نامه';
  if (url.startsWith('/venues')) return 'مشاهدهٔ رزرو';
...
export default function NotificationsPage() {
  ...
  const openNote = (note: Note) => {
    ...
    axiosService({ url: '/api/v1/notifications/read', method: 'post', token: token(), body: { id: note._id } })
  ...
  const readAll = () => {
    setBusy(true);
    axiosService({ url: '/api/v1/notifications/read', method: 'post', token: token(), body: {} })
      .then(() => load(0, true))
      .catch(() => setBusy(false));
  };
          ...
          <EmptyState
            ...
            sub={
              unreadOnly || kind !== 'all'
                ...
                : 'وقتی درخواستی ثبت یا پذیرفته شود، یا پیامی برسد، این‌جا خبردار می‌شوید.'
            }
          />
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: S.s2 }}>
