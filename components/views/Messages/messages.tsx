...
import { FileClock, MapPin, MessagesSquare, PhoneCall, Truck } from 'lucide-react';
...
/**
 * پیام‌ها — every conversation with a collector, in one place.
 *
 * Until now the chat existed only as a button on the card of the request it
 * belonged to. That is fine when you are already looking at the request, and
 * useless in the case that actually happens: a message arrives, and the person
 * has to guess which of their pickups it was about before they can read it.
 * This is the other direction — start from the message, and the request it
...
export default function MessagesPage() {
  ...
  const load = useCallback(() => {
    axiosService({ url: '/api/v1/chat/threads', method: 'get', token: Cookies.get('auth_token') })
          ...
          <EmptyState
            ...
            sub={
              filter === 'unread'
                ...
                : 'به‌محض اینکه جمع‌آوری درخواست شما را بپذیرد، این‌جا می‌توانید با او گفتگو کنید.'
            }
          />
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: S.s2 }}>
                  ...
                  <Card accent={unread ? C.statusInfo : undefined}>
                    <div style={{ padding: S.s4, display: 'flex', flexDirection: 'column', gap: S.s3 }}>
                      ...
                      <div style={{ display: 'flex', gap: S.s2, flexWrap: 'wrap' }}>
                        ...
                        <Link href="/history" style={{ flex: '1 1 96px', textDecoration: 'none' }}>
                          <span
                            style={{
