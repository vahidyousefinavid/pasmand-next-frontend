...
/**
 * The conversation about one pickup — the citizen's side of it.
 *
 * Both apps talk to the same chat endpoints and the same Message
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', justifyItems: 'center', gap: S.s2, padding: `${S.s6}px ${S.s4}px`, textAlign: 'center' }}>
            ...
            <p style={{ margin: 0, fontSize: S.xs, color: C.muted, lineHeight: 1.9, maxWidth: '34ch' }}>
              زمان دقیق مراجعه، نشانی یا جزئیات بار را همین‌جا بپرسید.
            </p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: S.s2 }}>
            {messages.map((m) => {
              const mine = m.senderModel === 'User';
              return (
